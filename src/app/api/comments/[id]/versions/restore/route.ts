...
/**
 * API Route: POST /api/comments/[id]/versions/restore
 * Restore comment to a specific version.
 *
 * Architecture:
 * - Thin controller layer
 * - Validates request body and path parameters
 * - Delegates to RestoreCommentVersionUseCase
 * - Creates new version after restoration
 *
 * Error Mapping:
 * - 400: Invalid version number
 * - 401: Unauthorized (not authenticated)
 * - 404: VERSION_NOT_FOUND, COMMENT_NOT_FOUND
...
export async function POST(
  ...
  try {
    ...
    // const useCase = new RestoreCommentVersionUseCase(
    //   new CommentRepositoryPrisma(),
    //   new ContentVersionRepositoryPrisma()
    // );
    // const comment = await useCase.execute(commentId, versionNumber);

    // TEMPORARY: Return mock response
    return NextResponse.json({
      success: true,
      message: `Restore comment version API route ready - would restore to version ${versionNumber}`,
      data: {
        id: commentId,
        content: `Comment content from version ${versionNumber}`,
    ...
    //     return NextResponse.json(
    //       { success: false, message: 'Cannot restore archived comment' },
    //       { status: 409 }
    //     );
    //   }
    // }
